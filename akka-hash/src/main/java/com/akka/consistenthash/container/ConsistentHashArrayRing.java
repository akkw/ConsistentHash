...
public class ConsistentHashArrayRing<T> extends ArrayList<Node.VirtualNode<T>> {
    ...
    @Override
    public Node.VirtualNode<T> get(int index) {
        rangeCheck(index);
        return binarySearch(index);
    }

    private Node.VirtualNode<T> binarySearch(int index) {
        // [n,m]
